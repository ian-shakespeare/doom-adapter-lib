...
// These forward declarations must match the actual parameter lists used
// at each function's real definition (in p_pspr.c/p_enemy.c). Native ABIs
// silently tolerate a declared/defined arity mismatch here, but wasm's
// call_indirect is strictly type-checked against the state table's
// actionf_t union (see d_think.h), so a mismatch here causes a wasm trap:
// "indirect call type mismatch". Two real signatures are used:
//   - actionf_p1: void (*)(mobj_t*)                     -- monster/fx actions
//   - actionf_p2: void (*)(player_t*, pspdef_t*)         -- player weapon actions
